...
class PelangganController extends SecureController{
	...
	function index($fieldname = null , $fieldvalue = null){
		...
		if(!empty($request->search)){
			...
			$search_condition = "(
				...
				pelanggan.user_id LIKE ? OR 
				pelanggan.alamat LIKE ? OR 
				pelanggan.no_telp LIKE ?
			)";
			$search_params = array(
				"%$text%","%$text%","%$text%","%$text%"
			);
		...
		$this->render_view("pelanggan/list.php", $data); //render the full page
	}
	/**
     * View record detail 
	...
	function add($formdata = null){
		if($formdata){
			...
			$request = $this->request;
			//fillable fields
			$fields = $this->fields = array("user_id","alamat","no_telp");
	...
	function edit($rec_id = null, $formdata = null){
		...
		$tablename = $this->tablename;
		 //editable fields
		$fields = $this->fields = array("id","user_id","alamat","no_telp");
		if($formdata){
			$postdata = $this->format_request_data($formdata);
			$this->rules_array = array(
				'user_id' => 'required|numeric',
