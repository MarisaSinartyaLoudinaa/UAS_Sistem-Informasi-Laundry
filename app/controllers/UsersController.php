...
class UsersController extends SecureController{
	...
	function add($formdata = null){
		if($formdata){
			...
			$request = $this->request;
			//fillable fields
			$fields = $this->fields = array("nama_customer","image","email","email_verified_at","password","is_admin","created_at","updated_at");
			...
			$db->where("nama_customer", $modeldata['nama_customer']);
			if($db->has($tablename)){
				$this->view->page_error[] = $modeldata['nama_customer']." Already exist!";
			}
			//Check if Duplicate Record Already Exit In The Database
	...
	function edit($rec_id = null, $formdata = null){
		...
		$tablename = $this->tablename;
		 //editable fields
		$fields = $this->fields = array("id","nama_customer","image","email_verified_at","is_admin","created_at","updated_at");
		if($formdata){
			$postdata = $this->format_request_data($formdata);
			$this->rules_array = array(
				'nama_customer' => 'required',
			...
			$modeldata = $this->modeldata = $this->validate_form($postdata);
			//Check if Duplicate Record Already Exit In The Database
			if(isset($modeldata['nama_customer'])){
				$db->where("nama_customer", $modeldata['nama_customer'])->where("id", $rec_id, "!=");
				if($db->has($tablename)){
					$this->view->page_error[] = $modeldata['nama_customer']." Already exist!";
				}
			} 
